...
std::string RETypeDefinition::get_full_name() const {
    ...
    // Don't cache the intermediate result here — only cache the final
    // full_name after all processing (generics, reflection fallback) is done.
    // Caching early would expose other threads to an incomplete name.
...
bool RETypeDefinition::is_enum() const {
    ...
    static std::once_flag enum_type_init{};
    static sdk::RETypeDefinition* enum_type = nullptr;
    std::call_once(enum_type_init, []() {
...
::REManagedObject* RETypeDefinition::get_runtime_type() const {
    ...
    // If we got here without finding the type, don't cache the negative result.
    // The type may become available later (e.g. after delayed assembly loading).
    // Previously this code cached nullptr which made transient failures permanent.
    return nullptr;
#else
    auto vm = sdk::VM::get();
...
// Cache for get_types_inherting_from_this
static std::shared_mutex g_inheriting_mtx{};
static std::unordered_map<const sdk::RETypeDefinition*, std::vector<sdk::RETypeDefinition*>> g_inheriting_map{};

std::vector<RETypeDefinition*> RETypeDefinition::get_types_inherting_from_this() const {
    {
        std::shared_lock _{g_inheriting_mtx};
        if (auto it = g_inheriting_map.find(this); it != g_inheriting_map.end()) {
            return it->second;
        }
    }
    ...
    std::unique_lock _{g_inheriting_mtx};
    g_inheriting_map[this] = std::move(out);
    return g_inheriting_map[this];
}
} // namespace sdk